...
figdata <- readRDS(file.path(data_dir, "(created) finaldata_260508.RDS"))
names(figdata)
...
figdata_corr <- figdata %>% 
  ...
  select(
    ...
    # CDI Components
    pr_female_hh, pr_pov, pr_pubassi, pr_unemprate
  ) %>%
...
png(
  file.path(figure_dir, "correlation_plot_subitems_260508.png"),
  width = 10, 
  height = 8, 
  units = "in", 
  res = 300
...
figdata2 <- readRDS(file.path(data_dir, "(created) finaldata_260508.RDS"))
...
figdata_corr2 <- figdata2 %>% 
  ...
  select(
    ...
    # # SVI composite indices - PCA
    # svi_overall_pca,      # Overall SVI
    # svi_theme1_pca,      # Theme 1: Socioeconomic Status
    # svi_theme2_pca,      # Theme 2: Household Characteristics
    # svi_theme3_z,      # Theme 3: Racial & Ethnic Minority Status
    # svi_theme4_pca,      # Theme 4: Housing Type & Transportation
    # CDI at different geographic levels
    disad_national_pca,  # National-level CDI
    disad_state_pca,     # State-level CDI
    disad_city_pca       # City-level CDI
  ) %>%
  filter(!is.na(crime_rate)) %>%
  rename(
    "Firearm Homicide Rate" = crime_rate,
    ...
    "CDI (National)" = disad_national_pca,
    "CDI (State)" = disad_state_pca,
    "CDI (City)" = disad_city_pca
  )

# Create correlation plot with composite indices
png(
  file.path(figure_dir, "correlation_plot_260508.png"),
  width = 10, 
  height = 8, 
  units = "in", 
  res = 300
...
saveWidget(
  map,
  file = file.path(figure_dir, "interactive_map_260508.html"),
  selfcontained = TRUE
)

cat("Interactive map saved to:", 
    file.path(figure_dir, "interactive_map_260508.html"), "\n")
